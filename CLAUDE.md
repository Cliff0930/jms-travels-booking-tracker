...
- **Gmail email threading:** `bookings.gmail_thread_id` is stored when a booking is created from an email (set in `parse-message` and `fill-missing`). `bookings.gmail_original_message_id` stores the RFC `Message-ID` header of the incoming email (set in `parse-message`). All outbound `sendEmailSafe` calls in `confirm`, `assign`, `resend`, `substitute`, and `legs/notify-client` pass both `replyToThreadId: booking.gmail_thread_id || undefined` AND `inReplyToMessageId: booking.gmail_original_message_id || undefined`. **Both are required**: Gmail API ignores `threadId` when the outgoing subject doesn't match the thread's subject — `In-Reply-To` is what actually makes Gmail honor the thread on both sender and recipient side. WhatsApp/manual bookings have `null` on both → new thread (no change).
...
- **Google Maps URLs from clients:** `pickup_location_url` / `drop_location_url` columns on `bookings` store map links sent by clients. On driver assignment (`assign/route.ts`), the URL is appended inside the `pickup`/`drop` params of `jms_trip_brief_driver` using ` | ` as separator (e.g. `"Address | Map: https://..."`). **Never use `\n` as separator** — Meta rejects newlines in template body params (error #132018). Never send as a separate free-form message — drivers may not have an open 24h window. Booking detail page and driver app both render these URLs as tappable map links.
  ...
  - **Google Search "share this place" links (fixed 2026-07-01, commit `5c48a7b`):** `extractMapsUrls()` (duplicated in `parse-message/route.ts` and `conversation/process.ts`) now also recognizes `google.com/search` URLs when they carry a `kgmid=` param (Google's Knowledge Graph ID — present on real place-share links from Android Search/Assistant, absent on unrelated searches). Found on BK-2026-0308: a `www.google.com/search?...&kgmid=...` link glued onto the address wasn't recognized, so the whole address+URL blob landed in `pickup_location` as raw text. Also added an explicit instruction to `EXTRACTION_PROMPT`/`CLASSIFY_AND_EXTRACT_PROMPT`/`CONVERSATION_PROMPT` to keep embedded URLs out of `pickup_location`/`drop_location` text.
...
- **Client profile phone + self-booking guest_phone fallback (fixed 2026-07-01, commit `5c48a7b`):** New clients auto-created from email never got `primary_phone` set at all, and when no distinct guest is named (sender IS the traveler — no separate guest mentioned), the signature-phone exclusion above left nowhere for their phone to land, forcing an avoidable "missing info" round-trip. Added `extractSenderPhone()` in `parse-message/route.ts` — finds labeled signature lines ("Mobile:", "Contact No:", etc.) — used to (1) populate `primary_phone` on new client creation, and (2) fill `guest_phone` only when `guest_name` came back null/empty (sender is genuinely the traveler). Does not touch the case where a distinct guest IS named but only the sender's own signature phone is available — that still correctly asks for the guest's own number.
...
### Time-correction messages misclassified as address updates (fixed 2026-07-01, commit `c763ab4`)
"Pickup time 10pm" overwrote `pickup_location` with that literal text and replied "address updated" instead of correcting the time. Root cause: `isLocationFollowUp`'s `noBookingSignals` keyword check used bare `\bam\b`/`\bpm\b`, which never matches compact notation like "10pm" — there's no word boundary between a digit and the following letter. A correct time-pattern regex already existed elsewhere in the same file (`hasTimePattern`, used for a different guard) but never got a chance to run since the broken check matched first and returned early.
...
- **`booking.status` for multi-leg trips:** When any leg completes (`leg_id` present, or Day 1's booking-level link with no `leg_id` — both treated as `day_number` lookups against the max non-cancelled leg), only the true last leg sets `completed`; if more legs remain, status stays `in_progress`. Fixed 2026-07-01 (commit `55f6bab`) — previously only the `leg_id` path had this check, so Day 1 always flipped `booking.status` to `completed` immediately regardless of remaining days, only self-healing once Day 2's arrived link was clicked.
...
- **Calendar/dashboard leg display (reversed 2026-07-01, commit `1d899dd`):** Continuation legs (Day 2+) always plot on the calendar/dashboard, regardless of overall `booking.status` — the previous filter (shipped 2026-06-25) that hid a leg once the booking left `confirmed`/`in_progress` and that leg hadn't run was removed per explicit request: users want every day of a multi-day trip visible with its own real status, even after the booking wraps up. Each leg's chip color is still derived from its own `leg_status` (`completed`/`in_progress`/fallback `confirmed`); only `leg_status === 'cancelled'` legs are skipped. Applied in `src/app/(dashboard)/bookings/calendar/page.tsx` and `src/app/(dashboard)/page.tsx` (`weekDays` `.some()` and `selectedDayTrips` `.find()`). Any new calendar/week view should follow the same "always show, status comes from leg_status" pattern.
- **Day 1 leg_status fix (2026-07-01, commit `315beec`):** `driver-status/route.ts` previously only updated `booking_legs.leg_status` for Day 1 (booking-level link, no `leg_id`) when `trip_type === 'outstation'`. Airport/local multi-day trips have the same booking-level Day 1 link but had no matching branch, so Day 1's `leg_status` stayed `upcoming` forever. Fixed by dropping the outstation-only condition — any booking-level link (no `leg_id`) now updates `day_number=1` for every trip type. Bookings that completed *before* this fix deployed have stale Day 1 `leg_status` and need a one-time manual backfill (`leg_status='completed'` via Supabase REST) to display correctly; the fix only affects new writes.
- **Booking status badge — per-day label (2026-07-01, commit `1eed7cb`):** `BookingStatusBadge` (`src/components/shared/StatusBadge.tsx`) takes an optional `label` prop. On the booking detail page, multi-day bookings (`total_days > 1`) compute `multiDayStatusLabel`: "Day X In Progress" while a leg is active, "Day X Completed" between legs, "Entire Trip Completed" only when `booking.status === 'completed'` AND every leg's `leg_status` is also `'completed'` (guards against the premature-completion quirk above making a false claim). Single-day trips are unaffected (label stays `undefined`, falls back to plain status text).
- **A stuck `in_progress` booking blocks a driver's OTHER links entirely:** `driver-redirect-check` Step 1 redirects every link a driver opens to any other `in_progress` booking's Completed page first, regardless of which booking they actually clicked. If that other booking never got properly closed (e.g. trip_type was changed after legs were generated for a different type, or the driver simply never submitted Completed), the driver sees "Trip Already Completed" on a brand-new link. Fix: close the stuck booking (ideally via the real "Complete Early" button, not "Force Status" override — see below) so it leaves `in_progress`. Root-caused on BK-2026-0294/0273, 2026-07-01.
- **"Complete Early" vs "Force Status" override — not the same thing:** Both can move a booking to `completed`, but only `POST /api/bookings/[id]/complete-early` also cancels remaining `upcoming` legs and completes any `in_progress` leg; `POST /api/bookings/[id]/override-status` (the Actions-panel "Force Status" buttons) only flips `booking.status` and leaves `booking_legs` untouched. Check `booking_status_history.changed_by` to tell which one actually ran: Complete Early logs `"Early completion: ..."`, Force Status logs plain `"operator"`.
- **Outstation `total_days` recalculation guard (2026-07-01, commit `9218be8`):** `driver-status/route.ts` recalculates `total_days` from elapsed days between `trip_opening_date`/`trip_closing_date` on outstation completions — but only writes it when `newBookingStatus === 'completed'` (i.e. this is genuinely the last remaining leg). Without this guard, completing an early leg on a booking that still has other legs left (e.g. a stale multi-leg structure left over from a trip_type change — see below) silently shrinks `total_days` mid-trip.
- **`trip_type` change doesn't reconcile existing legs — now warned, not silent:** Outstation bookings need exactly 1 leg, but changing `trip_type` to `'outstation'` on a booking that already has legs from a different type (e.g. a 4-day attached-vehicle booking later reclassified as outstation) doesn't collapse them automatically. `bookings/[id]/edit/route.ts` now returns a `warning` field (shown as a toast) when this mismatch is detected — legs are NOT auto-modified since they may already have real `trip_sheets` data attached; reconcile manually via the Trip Legs panel.
- **Trip opening/closing dates captured for every trip type (2026-07-01, commits `f6183c8`, `0af097d`, `3c4e81a`):** `trip_sheets.trip_opening_date`/`trip_closing_date` are set on every Arrived/Completed action for any trip type (previously outstation-only) — needed for accurate "Total Hours" display whenever a trip spans a midnight boundary by more than the old same-day/single-wrap assumption could handle (e.g. a local job opened Day 1, closed Day 2). Shown on the booking detail page's Actual/Driver/Client tripsheet tabs (editable; read-only row only appears when the two dates differ, to avoid clutter on ordinary same-day trips) and on the driver's Completed form ("Closing Date", defaults to today, editable for any trip type). `total_days` recalculation itself stays outstation-only. Confirmed this was always a display-only bug — billing generation never used the buggy same-day hour calc for outstation (`calcOutstationTrip` uses `total_days` directly), so no invoices/settlements were ever wrong.
...
- **PostgREST two-FK ambiguity — CRITICAL:** When a table has two or more FKs pointing to the same target table, PostgREST cannot resolve un-hinted embeds and **errors on every query** that uses them — breaking the entire feature. Always use `!column_id` hint: `company:companies!company_id(...)` not `company:companies(...)`. Real incident (2026-06-27): adding `billing_company_id UUID REFERENCES companies(id)` to `bookings` created a second FK alongside `company_id` — all 14 booking routes broke because `company:companies(...)` was ambiguous. When adding a new FK to a table that already references the same target, immediately grep for all un-hinted embeds in routes that query that table and add `!fk_column` to each. Also always include `NOTIFY pgrst, 'reload schema';` after any `ALTER TABLE ... ADD COLUMN ... REFERENCES ...` migration.
