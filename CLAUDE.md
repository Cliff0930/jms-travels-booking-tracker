...
- **Alert indicators (all three views):** Calendar day cells, dashboard WeekDayCard tiles, and bookings list BookingCard all show the same alert system: pulsing dot on day cells + `⚠ No Driver Assigned` (red border + tag) / `⚠ Draft — Confirm` (amber border + tag) on individual booking cards. Dashboard BookingTile and bookings page BookingCard both use `!border-l-4 !border-l-red-500` / `amber-500` left border + inline `⚠` tag. BookingCard border priority: needsClarification (orange) > noDriver (red) > possibleDup/isDraft (amber) > default.
