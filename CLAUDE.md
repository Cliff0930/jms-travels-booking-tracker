...
- **Duplicate guard can swallow correction emails:** If a client sends a correction email minutes after the booking email ("cancel Innova, all 4 Sedan"), and Gemini classifies the correction as `booking` (it reads the full quoted thread), the duplicate guard fires and marks it `ai_classification = 'duplicate'`. The fix is silently dropped. When a booking has unexpected data, always check `raw_messages` for `ai_classification = 'duplicate'` entries from the same sender in the same time window — they may contain corrections that need manual application.
...
- **WhatsApp "I sent a mail" blocks the real email:** When a client says "I have sent a mail on the same" on WhatsApp, the bot creates a booking with that text as `pickup_location`. When the real email arrives with proper details, the duplicate guard blocks it (`ai_classification = 'duplicate'`). Recovery: manually PATCH the booking from the email content; check raw_messages for the dropped email.
