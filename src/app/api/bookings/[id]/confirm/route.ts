  ...
  if (!skipNotification) {
    ...
    if (booking.source === 'email') {
      ...
      if (phones.length > 0) {
        ...
        channel = 'email'
        recipient = email
        const result = await sendEmailSafe({ to: email, subject: `Your booking is confirmed - ${booking.booking_ref}`, body, booking_id: id, replyToThreadId: booking.gmail_thread_id || undefined })
