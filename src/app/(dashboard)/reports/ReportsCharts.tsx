...
interface Props {
  ...
  tripTypeCounts: ChartEntry[]
  topCompanies: ChartEntry[]
  byDate: DateEntry[]
}

export default function ReportsCharts({ statusCounts, sourceCounts, tripTypeCounts, topCompanies, byDate }: Props) {
  return (
    <div className="space-y-4 mb-5">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-lg border border-[#C3C5D7] p-4">
          <p className="text-sm font-semibold text-[#191B23] mb-3">By Trip Type</p>
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
              <Pie data={tripTypeCounts} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={55} label={({ name, value }) => `${name}: ${value}`} labelLine={false}>
                {tripTypeCounts.map((entry, i) => <Cell key={i} fill={entry.fill} />)}
              </Pie>
              <Tooltip />
            </PieChart>
          </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-lg border border-[#C3C5D7] p-4">
          <p className="text-sm font-semibold text-[#191B23] mb-3">Top Companies</p>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={topCompanies} layout="vertical" margin={{ left: 8 }}>
              <XAxis type="number" allowDecimals={false} tick={{ fontSize: 10 }} />
              <YAxis type="category" dataKey="name" tick={{ fontSize: 10 }} width={110} />
              <Tooltip />
              <Bar dataKey="value" fill="#7B5EA7" radius={[0, 3, 3, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
