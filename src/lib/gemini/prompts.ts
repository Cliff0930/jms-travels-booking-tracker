...
11. total_days — number of days if outstation (default 1). "Attached" or "attached vehicle" = dedicated multi-day local booking; set trip_type="local" and total_days to the number of days mentioned. For attached bookings, pickup_time and pickup_location are NOT mandatory if not provided — create the booking with whatever is given; the driver or operator will coordinate the daily details on-ground. If a start date (pickup_date) and a separate end date are both given via "till/until/upto/up to [date]" (e.g. pickup_date is 1 July and the message says "vehicle required till 4th July"), calculate total_days as the inclusive day count between the two dates (1 July to 4 July = 4 days — count carefully, do not guess).
12. special_instructions — brief notes about THIS booking only: explicit end times — "till", "until", "upto"/"up to" followed by a CLOCK TIME (not a date) all mean the same thing: how long the vehicle/driver should be on standby, NOT the pickup_time. E.g. "till 1800 hrs" / "upto 10:00 PM" / "until 6pm" → special_instructions = "Vehicle required till 18:00" / "Vehicle required until 22:00". If no other clock time is given anywhere in the message, pickup_time stays null and is added to missing_mandatory — never use an end-time phrase as pickup_time. Also covers: follow-up instructions, ministry/department affiliation, billing notes. Max 200 characters. Do NOT copy in booking requests for other guests or other trips from the same message.
...
=== LOCATION KEYWORDS ===
If the client uses a shorthand like "home", "office", "residence", "factory" check their saved_locations. If found → use the saved address. If not found → check whether an earlier client message in the conversation contains a specific street address, building name, or PIN code that is clearly the same place. If yes, resolve the shorthand to that specific address. Only if no address appears in history: accept the shorthand as-is (e.g. "Home", "Residence") and proceed. Do NOT ask the client to clarify the address — the driver or operator will confirm it directly.
