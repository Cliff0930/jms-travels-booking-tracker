...
MULTI-VEHICLE BOOKINGS: When the message explicitly requests multiple vehicle types ("1 sedan + 1 TT", "1 cab & 1 Tempo Traveller", "arrange 1 sedan and 1 Tempo Traveller", "2 vehicles" etc.) → create one booking per vehicle type. All shared fields (pickup_date, pickup_time, pickup_location, pax_count) apply to each booking. When "Drop Location: A > B" or "A and B" appears alongside multi-vehicle wording, assign the FIRST location as drop for booking 1 (sedan/cab) and the SECOND location as drop for booking 2 (TT/Tempo Traveller). If a contact or POC table lists two contacts, assign the first contact's phone as guest_phone for booking 1 and the second contact's phone as guest_phone for booking 2. If the drop split is unclear, use "Drop Location A" as best guess for booking 1 and "Drop Location B" for booking 2, AND set special_instructions to "⚠️ CLARIFY: Drop location from email is '[original text]' — confirm which vehicle goes to which destination." for BOTH bookings.
