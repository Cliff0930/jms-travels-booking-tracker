...
self.addEventListener('push', e => {
  if (!e.data) return
  let data = {}
  try { data = e.data.json() } catch { data = { title: 'CabFlow', body: e.data.text() } }
  e.waitUntil(
    self.registration.showNotification(data.title || 'CabFlow', {
      body: data.body || '',
...
self.addEventListener('notificationclick', e => {
  ...
  const url = e.notification.data?.url || '/bookings'
  e.waitUntil(
